    ...
    async def init(self):
        ...
        if not db_path.parent.exists():
            try:
                ...
                logger.critical(f"Failed to create database directory {db_path.parent}: {e}")
                sys.exit(1)
...
class ProxyFetcher:
    ...
    def __init__(self, args: argparse.Namespace):
        self.args = args
        db_file = Path(args.db_path) / args.db_fname
        self.db = ProxyDB(path=str(db_file))
    ...
    async def _establish_pivot(self) -> bool:
        ...
        if self.args.pivot:
            ...
            for url in manual_urls:
                try:
                    # Robust parsing: handle scheme://ip:port or ip:port
                    ...
                    p = Proxy(proto.lower(), ip, port, found_via="MANUAL_PIVOT")
...
"""
        if self.args.append_tor:
            ...
            logger.info("Caught `--append-tor`, appended `socks5  127.0.0.1  9050` to config...")
        ...
        try:
            ...
            if self.args.output_path:
                out_dir = Path(self.args.output_path)
                if not out_dir.exists():
                    out_dir.mkdir(parents=True, exist_ok=True)
                output_path = out_dir / output_path
            
            output_path.write_text(config)
            logger.info(f"Saved {len(self.working)} working proxies to {output_path}")
        except PermissionError:
            logger.error(f"Permission denied: Could not write proxychains config to {output_path}")

        # JSON Export
        if self.args.json or self.args.update_json or self.args.validate_json:
            path_str = self.args.json or self.args.update_json or self.args.validate_json
            path = Path(path_str)
            if self.args.output_path:
                out_dir = Path(self.args.output_path)
                out_dir.mkdir(parents=True, exist_ok=True)
...
async def main():
    parser = argparse.ArgumentParser(description="NGF v0.1.4 - Advanced Proxy Fetcher")
    ...
    # Configuration summary display using ASCII pipes
    border = "+" + "-" * 67 + "+"
    ...
    if args.db_dump or args.db_count or args.db_json or args.db_import or args.db_clear:
        ...
        try:
            ...
            if args.db_json:
                ...
                out_path = Path(args.db_json)
                if args.output_path:
                    out_dir = Path(args.output_path)
                    out_dir.mkdir(parents=True, exist_ok=True)
                    out_path = out_dir / out_path
                out_path.write_text(json.dumps(export, indent=2))
                CONSOLE.print(f"[bold green]✓[/] Database exported to [bold white]{out_path}[/] ({len(results)} entries)")
